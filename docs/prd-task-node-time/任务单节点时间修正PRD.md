...
问题处理：后台修改电子围栏后，需要将新的围栏数据同步至已派发的任务单中，以解决司机因GPS漂移或处于围栏边缘而无法打卡的问题
...
| 角色 | 使用场景 |
| --- | --- |
| 调度员 | 修正任务节点时间、修正司机及车辆信息、修正司机操作错误导致的数据异常 |
...
| 修改任务信息 | 驾驶员信息修改 | 修改任务单主司机、牵引车、挂车 |
...
| 一级功能 | 二级功能 | 说明 |
| --- | --- | --- |
| 司机任务 | 任务详情同步 | 展示后台修正后的节点时间、司机及车辆信息、磅单信息 |
...
当任务单属于多线路拆分任务时，用户仍只修改当前任务单的装到达时间、装离开时间、卸到达时间、卸离开时间。
系统保存时，除校验当前任务单内部时间外，还需要按照线路实际拆分顺序，校验当前任务与上一段、下一段任务的时间关系。
系统每次只修改当前任务单，不自动修改上一段或下一段任务的时间
...
| 原牵引车 | 文本 | 是 | 否 | 否 | 当前任务绑定牵引车，置灰不可修改 |
| 原挂车车牌号 | 文本 | 是 | 否 | 否 | 当前任务绑定挂车，置灰不可修改 |
| 新司机 | 下拉选择/搜索选择 | 是 | 是 | 否 | 从司机档案中选择 |
| 新司机电话 | 自动带出 | 是 | 否 | 否 | 根据新司机自动带出 |
| 新牵引车牌 | 下拉选择/搜索选择 | 是 | 是 | 否 | 从车辆档案中选择 |
| 新挂车牌 | 下拉选择/搜索选择 | 是 | 是 | 否 | 从挂车档案中选择 |

#### 驾驶员信息交互规则

- 打开弹窗时，系统默认展示当前司机信息。
- 原司机、原司机电话、原牵引车、原挂车车牌号置灰展示，不允许修改。
- 用户点击「新司机」字段，可按司机姓名、手机号搜索司机。
- 选择新司机后，系统自动带出司机姓名、电话。
- 如果选择的新司机状态为停用，驾驶员筛选器不带出值。
- 如果新司机与原司机相同，视为未修改。
- 驾驶员信息可与节点时间、磅单信息同时修改。
- 保存时系统自动记录原司机、新司机、原牵引车、新牵引车牌、原挂车车牌号、新挂车牌信息。
- 修改成功后，任务单列表、任务单详情、小程序任务详情同步展示新司机、牵引车、挂车信息。

![文档图片](assets/image2-vehicle-update.svg)
...
| 备注说明 | 用户填写的备注 |
| 修改字段 | 被修改的字段 |
| 修改前值 | 修改前时间、司机、牵引车、挂车或磅单信息 |
| 修改后值 | 修改后时间、司机、牵引车、挂车或磅单信息 |
...
| 多线路拆分时间 | 上一段任务最后一个已有节点时间≤当前任务第一个已有节点时间 | 当前任务时间不能早于上一段任务的结束时间 |
| 多线路拆分时间 | 当前任务最后一个已有节点时间≤下一段任务第一个已有节点时间 | 当前任务时间不能晚于下一段任务的开始时间 |
| 多线路拆分时间 | 相邻任务没有节点时间时，跳过对应的前后任务校验 | 无须提示，不影响保存 |
